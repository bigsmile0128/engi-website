...
import ShareModal from '~/components/JobDetails/ShareModal';
...
export default function JobDetails() {
  ...
  const { id } = useParams();
  // TODO: convert to use Remix instead of react-query
  // TODO: remove from tsconfig exclude
  ...
  return isError ? (
    <div className="flex flex-col items-center justify-center py-24">
      <p className="text-center font-grifter text-3xl">
        Something went wrong...
      </p>
    </div>
  ) : (
    <div className="mx-auto flex max-w-7xl flex-col items-start gap-x-12 p-8 sm:p-16 md:p-24 lg:flex-row">
      ...
      <div className="flex flex-1 flex-col">
        <div className="flex items-center">
          ...
          <h1
            className={classNames(
              'px-4 font-grifter text-3xl',
              isLoading ? `skeleton mx-2 animate-pulse` : ''
            )}
          >
        ...
        <div className="mt-12 flex items-center gap-x-4">
          {isLoading ? (
            <Tag className="skeleton border-transparent !py-2">
        ...
        <h2
          className={classNames(
            'self-start font-grifter text-2xl',
            isLoading ? 'skeleton' : ''
          )}
        >
          Job Description
        ...
        <div className="mt-6 mb-12 text-gray-300">
          ...
          >
            <div className={`skeleton w-5/5 h-5 animate-pulse`} />
            <div className={`skeleton h-5 w-3/5 animate-pulse`} />
            <div className={`skeleton h-5 w-4/5 animate-pulse`} />
            <div className={`skeleton h-5 w-3/5 animate-pulse`} />
          </div>
          <span className={isLoading ? 'hidden' : ''}>{data?.description}</span>
        </div>
        <div className="flex flex-col gap-x-6 gap-y-6 md:flex-row md:items-center">
          <PaymentInfo
      ...
      <JobCreatorInfo
        className="mt-6 w-full shrink-0 md:w-96 lg:mt-0 lg:w-auto lg:basis-80"
        isLoading={isLoading}
      />
    </div>
  );
